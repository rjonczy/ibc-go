	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.coordinator.Setup(path)
				expSeq = 1
				suite.chainA.App.GetIBCKeeper().ChannelKeeper.SetNextSequenceSend(suite.chainA.GetContext(), path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID, expSeq)
